...
::: {.callout-note}
This derivative is only correct if $k$ is itself a constant!
:::
...
```{julia}
# This struct is the same thing as FNode, but we'll give it a new name
# to avoid confusion.
struct ForwardADResult
    value::Float64
    deriv::Float64
end
...
```{julia}
df_dx = _f2(x_val, 1.0, y_val, 0.0).deriv
df_dy = _f2(x_val, 0.0, y_val, 1.0).deriv
...
function _transform_single_expr(expr::Expr)
    # `return x` -> `return ForwardADResult(x, dx)`
    MT.@capture(expr, return retval_) && return :(return $ForwardADResult($retval, $(d(retval))))
...
df_dx = f2(x_val, 1.0, y_val, 0.0).deriv
df_dy = f2(x_val, 0.0, y_val, 1.0).deriv
...
Thankfully, for the purposes of this tutorial we will ignore such a possibility.
:::

Thus, for example, if we were trying to differentiate a function `f(x1, x2, x3)`, then the appropriate signature would be

```julia
function ChainRulesCore.frule(
    (fdot, x1dot, x2dot, x3dot),
    ::typeof(f),
    x1,
    x2,
    x3
...
Let's try to hook into this rule system now so that we don't have to do it ourselves.
...
function _transform_single_expr(expr::Expr)
    ...
    ####################################################
    ### We can use ChainRules.jl to handle all unary functions
    ####################################################
    if MT.@capture(expr, lhs_ = f_(rhs_))
        return :(($lhs, $(d(lhs))) = frule((NoTangent(), $(d(rhs))), $f, $rhs))
    end
...
::: {.callout-note}
In this example, we have set `fdot` to _always_ be a sentinel value of `NoTangent()`.
This is a special value that ChainRulesCore.jl uses to indicate that there is no meaningful way to differentiate `f` itself; however, it will fail for cases where `f` itself contains differentiable data.
:::

We could show that this works by running our old function, but to demonstrate that this actually uses the ChainRules.jl rules, we can instead try it out on a new function that uses the `cos` and `tan` functions, for which we haven't defined any rules ourselves.
As mentioned earlier, the actual rules themselves are defined in ChainRules.jl, so we need to import it.

```{julia}
using ChainRules

@forward_ad function f3(x::Float64, y::Float64)
    p = cos(x)
    q = tan(y)
    f = p + q
    return f
...
df_dx = f3(x_val, 1.0, y_val, 0.0).deriv
df_dy = f3(x_val, 0.0, y_val, 1.0).deriv
(df_dx, df_dy)
```

Let's compare this with the analytic derivative: for $f_3(x, y) = \cos(x) + \tan(y)$, we have that $\deriv{f_3}{x} = -\sin(x)$ and $\deriv{f_3}{y} = \sec^2(y)$.

```{julia}
(-sin(x_val), sec(y_val)^2)
```
